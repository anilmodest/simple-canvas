...
public class Line implements IShape {
    ...
    public void Draw(DrawingBoard drawingBoard) throws Exception {
        ...
        if(this.isHorizontalLine()) {
            ShapeUtility.getHorizatalLinePoints(Integer.min(this.x1, this.x2), Integer.max(this.x1, this.x2) + 1, this.y1, LINE_CHAR).forEach(dp -> drawingBoard.setPoint(dp));
        }
        else if(this.isVerticalLine()) {
            ShapeUtility.getVerticalLinePoints(Integer.min(this.y1, this.y2), Integer.max(this.y1, this.y2) + 1, this.x1, LINE_CHAR).forEach(dp -> drawingBoard.setPoint(dp));
        }
        else{
            throw new Exception(ExceptionMessages.ONLY_HORIZONTAL_VERTICAL_LINES_SUPPORTED);
        }
